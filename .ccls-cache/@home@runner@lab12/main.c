...
void fillArray( int pArray[], int size)
{
    ...
    for( int i=0; i<=size; i++) {
        ...
        for( insertIndex = 0; insertIndex < i && randValue >= pArray[ insertIndex]; insertIndex++)
            ;
...
//---------------------------------------------------------------------------
// Repeatedly remove random elements from array, shifting others down, until it is empty.
...
double useLinkedList( int size)
{
  ...
  double elapsedTime = difftime(time( NULL), beginTime);
    ...
    // Free the array enclosing all the Node * that previously pointed to the Nodes
    free( pNodes);
